...
func init() {
	...
	orm.RegisterDataBase("default", "mysql", "root:159+357mhq@tcp(127.0.0.1:3306)/test?charset=utf8", 30)

	// register model
	orm.RegisterModel(new(User),new(Area),new(Facility),new(HouseImage),new(OrderHouse),new(House))

	// create table
	orm.RunSyncdb("default", false, false)//第二个参数：false 表不更新 ， true 表更新 第三个参数： true 表创建 ， false 表不创建
}